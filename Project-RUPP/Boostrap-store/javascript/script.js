document.addEventListener("DOMContentLoaded", () => {
  ...
  function DisplayProduct(container, products) {
    ...
    products.forEach((product) => {
      const { image, names, rate, price, brand } = product;
      container.innerHTML += `
        <div class="col p-2">
  ...
  fetch("../Data/Datas.json")
    ...
    .then((data) => {
      ...
      allProducts = data.Product.filter((product) => product.category === pageType||product.subCategory === pageType); // Filter products by page type
      }
  ...
  function applyFilters() {
    ...
    // Filter by color
    const selectedColor = [...colorFilters].find((btn) => btn.classList.contains("active"))?.dataset?.color;
    if (selectedColor) {
  ...
  brandFilter.addEventListener("click", (e) => {
    if (e.target.tagName === "A") {
      e.preventDefault();
      brandFilter.querySelectorAll("a").forEach((link) => link.classList.remove("active"));
