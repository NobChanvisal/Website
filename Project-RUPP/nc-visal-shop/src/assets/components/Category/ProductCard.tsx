import { Link } from "react-router-dom"; // Import Link
import { Product } from "../interfaces/ProductType";
interface ProductCardProps {
  product: Product;
}

const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  return (
    <div className="product-card w-full relative outline outline-transparent hover:outline-1 hover:outline-[#229799]">
      <Link to={`/product/${product.id}`} aria-label="View product details"> 
        <img className="w-fit h-fit" src={product.img} alt={product.title} />
      </Link>
      <div className="product-info flex flex-col pt-4 pl-1">
        {product.prevPrice > 0 && (
          <span className="discount absolute right-0 top-0 px-3 my-2 mx-3 bg-red-500 text-white rounded-sm">
            Sale
          </span>
        )}
        <span className="brand mr-auto uppercase text-[#0656b6] font-semibold mb-2">
          {product.brand}
        </span>
        <Link to={`/product/${product.id}`} className="product-title text-[18px] text-[#191919] font-semibold">
          {product.title}
        </Link>
        <p className="category text-[14px] text-[#676d75] pb-2">
          {product.category}
        </p>
        <div className="rating w-[20%] pb-2">
          <img src={product.stars} alt={`${product.stars} stars`} />
        </div>
        <div className="price-content flex pb-2">
          <p className="new-price text-[#191919] pr-3">${(product.salePrice/100).toFixed(2)}</p>
          {product.prevPrice > 0 && (
            <p className="prev-price line-through text-[#676d75]">
            ${(product.prevPrice/100).toFixed(2)}
          </p>
          )}
        </div>
      </div>
    </div>
  );
};

export default ProductCard;
